...
-  NEXT_PUBLIC_APP_URL=https://nihemart.rw # used to build redirect URLs locally
...
If SMTP isn't configured the server will still generate backend action links (password reset / signup links) and return a warning. This lets you test link generation without immediately having SMTP credentials.

Riders and unconfirmed emails

-  The existing admin create-rider flow (server-side) uses the backend admin API to create a user and sets `email_confirm: true` which allows rider accounts to sign in immediately even if they haven't confirmed via email.
