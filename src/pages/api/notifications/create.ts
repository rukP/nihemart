...
export default async function handler(
   ...
   if (type === "order_cancellation_requested") {
      try {
         const tmeta = meta as NotificationMeta | undefined;
         if (
            tmeta &&
   ...
   try {
      ...
      switch (type) {
         ...
         case "order_created": {
            ...
            if (isAdmin) {
               ...
               finalBody = `${customerName} placed an order (${items
                  .map((it: any) => it.product_name || it.productName || "item")
                  .slice(0, 5)
                  .join(", ")}${
         ...
         case "order_created": {
            ...
            if (isAdmin) {
               // Short but informative admin notification - include variants for precision
               finalTitle = `New Order - ${orderNumber}`;
               const itemsList = items
                  .map((it: any) => {
                     const itemName =
                        it.variation_name || it.variationName
                           ? `${
                                it.product_name ||
                                it.productName ||
                                it.name ||
                  ...
                  .slice(0, 5);
               finalBody = `${customerName} placed an order (${itemsList.join(
                  ", "
               )}${items.length > 5 ? ` and ${items.length - 5} more` : ""})`;
            } else {
         ...
         case "order_cancellation_requested": {
            ...
            if (isAdmin) {
               finalTitle = `Order Cancellation Requested - ${orderNumber}`;
               let cb = `Customer ${customerName} requested cancellation for ${orderNumber}.`;
               if (metaTyped.reason) cb += `\n\nReason: ${metaTyped.reason}`;
               if (items && items.length > 0)
                  cb += `\n\nItems:\n${items
                     .map(
                        (it) =>
               ...
               finalTitle = `Cancellation Request Received - ${orderNumber}`;
               let cb = `We received your request to cancel ${orderNumber}. Our team will review and confirm shortly.`;
               if (items && items.length > 0)
                  cb += `\n\nItems:\n${items
                     .map(
                        (it) =>
