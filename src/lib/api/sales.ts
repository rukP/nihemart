...
async function fetchAllOrders(
  ...
  while (hasMore) {
    ...
    const total = ordersRes?.pagination?.total || ordersRes?.count || orders.length;
...
export async function getSalesChartData(
  ...
  deliveredOrders.forEach((order: any) => {
    const orderDate = new Date(order.createdAt || order.created_at || order.CreatedAt);
...
export async function getSalesByCategory(
  ...
  deliveredOrders.forEach((order: any) => {
    ...
    items.forEach((item: any) => {
      ...
      const itemTotal = Number(item.total || item.Total || item.price * (item.quantity || 0) || 0);
...
export async function getSalesByRegion(
  ...
  deliveredOrders.forEach((order: any) => {
    const region = order.deliveryCity || order.delivery_city || order.DeliveryCity || "Unknown";
    const orderId = order.id || order.Id || "";
    const userId = order.userId || order.user_id || order.UserId;
    const orderTotal = Number(order.total || order.Total || 0);
