...
export async function fetchStoreProductById(
   ...
   if (typeof window !== "undefined") {
      try {
         ...
         console.warn("store.fetchStoreProductById cache lookup failed", err);
      }
   }
   ...
   try {
      ...
      try {
         ...
         if (err?.response?.status === 404) {
            try {
               const resp2 = await unauthorizedAPI.get(
                  `/products/${productId}`,
               );
               result = resp2.data;
            } catch (err2: any) {
               ...
               try {
                  ...
                  try {
                     const allProductsResponse = await handleApiRequest(() =>
                        unauthorizedAPI.get(
                           "/products/store?limit=10000&page=1",
                        ),
                     );
...
export async function searchProductsByName(
   ...
   try {
      ...
      if (Array.isArray(data) && data.length > 0) {
         return data.map((p: any) => ({
            ...
            short_description:
               p.short_description ?? p.shortDescription ?? null,
            category: p.category ?? null,
         }));
      }
   } catch (err) {
      // If server search fails, fall back to local cached search
      console.warn("searchProductsByName server search failed", err);
   }
   ...
   if (typeof window !== "undefined") {
      try {
         ...
         console.warn("searchProductsByName cache lookup failed", err);
      }
   }
...
export async function getLandingPageProducts(): Promise<StoreProduct[]> {
   if (typeof window !== "undefined") {
      try {
         ...
         console.warn("getLandingPageProducts cache lookup failed", err);
      }
   }
