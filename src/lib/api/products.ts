...
const STOREPRODUCTS_CACHE_TTL = 1000 * 60 * 5; // 5 minutes
...
export async function fetchAllStoreProductsCached(forceRefresh = false) {
  ...
  if (
    !forceRefresh &&
    _storeProductsCache &&
    Date.now() - _storeProductsCacheFetchedAt < STOREPRODUCTS_CACHE_TTL
  ) {
  ...
  if (options.filters) {
    ...
    if (options.filters.rating) {
      filtered = filtered.filter(
        (p: any) =>
          (p.average_rating ?? p.averageRating ?? 0) >= options.filters!.rating!
      );
    }
  }
