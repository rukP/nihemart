...
export async function fetchSubcategories(options?: {
  ...
  try {
    ...
    console.log('[fetchSubcategories] Fetching from /categories/with-subcategories');
    
    const response = await handleApiRequest(() => unauthorizedAPI.get('/categories/with-subcategories'));
  ...
  if (options?.category_id) {
      ...
      console.log(`[fetchSubcategories] Filtered to ${filteredSubcategories.length} subcategories for category ${options.category_id}`);
    }
    ...
    console.log('[fetchSubcategories] Final result:', transformedSubcategories.length, 'subcategories');
...
export async function fetchCategories(options?: {
  ...
  try {
    ...
    const response = await handleApiRequest(() =>
      authorizedAPI.get(`/categories/admin${queryString ? `?${queryString}` : ""}`)
    );
...
export async function createCategory(data: {
  ...
  return handleApiRequest(() =>
    authorizedAPI.post("/categories/admin", data)
  );
}
