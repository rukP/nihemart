...
export const PAYMENTMETHODS = {
  mtn_momo: {
    ...
    bankId: BANKCODES.MTNMOMO,
    icon: '📱',
  },
  airtel_money: {
    code: 'momo',
    name: 'Airtel Money',
    bankId: BANKCODES.AIRTELMONEY,
    icon: '📱',
  },
  visa_card: {
    code: 'cc',
    name: 'Visa Card',
    bankId: BANKCODES.VISAMASTERCARD,
    icon: '💳',
  },
  mastercard: {
    code: 'cc',
...
export class KPayService {
  ...
  async initiatePayment(params: {
    ...
    const paymentConfig = PAYMENTMETHODS[params.paymentMethod];
  ...
  private async requestWithRetries(
    ...
    const alternate =
      url === (process.env.KPAYLIVE_BASE_URL || 'https://pay.esicia.rw')
        ? this.config.baseUrl
        : process.env.KPAYLIVE_BASE_URL || 'https://pay.esicia.rw';
        ...
        const isDnsOrConnectError = !!(
          (cause &&
            (cause.code === 'ENOTFOUND' ||
              cause.code === 'UNDERR_CONNECT_TIMEOUT')) ||
...
export function initializeKPayService(): KPayService {
  const config: KPayConfig = {
    baseUrl: process.env.KPAYBASE_URL || 'https://pay.esicia.com',
    username: process.env.KPAYUSERNAME || '',
    password: process.env.KPAYPASSWORD || '',
    retailerId: process.env.KPAYRETAILER_ID || '',
    environment:
      (process.env.KPAYENVIRONMENT as 'sandbox' | 'live') || 'sandbox',
    webhookUrl:
