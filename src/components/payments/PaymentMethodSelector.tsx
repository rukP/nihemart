...
export interface PaymentMethodSelectorProps {
  selectedMethod: keyof typeof PAYMENTMETHODS | 'cash_on_delivery';
  onMethodChange: (
    method: keyof typeof PAYMENTMETHODS | 'cash_on_delivery'
  ) => void;
  disabled?: boolean;
  className?: string;
  // Mobile Money
...
export default function PaymentMethodSelector({
  ...
  const paymentOptions: (keyof typeof PAYMENTMETHODS | 'cash_on_delivery')[] = [
    'mtn_momo',
    'airtel_money',
    'visa_card',
    'mastercard',
  ...
  const handleMobileMoneyPhoneConfirm = (phoneNumber: string) => {
    ...
    if (method && onMobileMoneyPhoneChange) {
      ...
      // Only change method if it wasn't already selected (i.e., not editing)
                ...
                {(method === 'mtn_momo' || method === 'airtel_money') &&
                  mobileMoneyPhones[method] && (
                    <div className="flex items-center gap-2 mt-0.5">
                      ...
                      {onMobileMoneyPhoneChange && !disabled && (
                        <Button
                          type="button"
                          variant="ghost"
                          size="sm"
                          ...
                          onClick={e =>
                            handleEditPhoneNumber(
                              e,
                              method as 'mtn_momo' | 'airtel_money'
                            )
                          }
