...
const CustomerTrendGraph = () => {
  ...
  const chartData = useMemo(() => {
    ...
    if (statsData && Array.isArray(statsData.daily) && statsData.daily.length) {
    ...
    const orders = Array.isArray(ordersResponse)
      ? ordersResponse
      : ordersResponse?.data || [];
    if (ordersResponse && Array.isArray(orders)) {
      ...
      for (const o of orders as any[]) {
        ...
        if (isWeekly) {
          ...
          // Group by date in local YYYY-MM-DD format
        ...
        <CardContent className="px-0">
          <div className="h-80">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={chartData} margin={{ top: 20, left: -20 }}>
                <defs>
