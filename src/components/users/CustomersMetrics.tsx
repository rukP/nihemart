...
const CustomersMetrics: FC<CustomersMetricsProps> = ({}) => {
  const { loading: usersLoading, roleCounts } = useUsers();
  ...
  const { data: ordersResponse, isLoading: ordersLoading } = useAllOrders({
    filters: {
      ...(dateRange.from ? { dateFrom: formatLocalDate(dateRange.from) } : {}),
  ...
  const metrics = useMemo(
    ...
    [totalCustomers, totalOrders, completedOrders, totalSales, completionRate]
  );
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card>
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-600 mt-1">Pending + Processing</p>
          </CardContent>
        </Card>
