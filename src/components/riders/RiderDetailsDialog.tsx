...
export default function RiderDetailsDialog({
   ...
   const metrics = React.useMemo(() => {
      ...
      // Determine bounds for fee adjustments (respect explicit range when set)
      ...
      for (const adjustment of feeAdjustments) {
         // Use transactionDate for accurate date filtering, fallback to createdAt for old records
         ...
         if (!adjDate) continue;
         if (adjStart && adjDate < adjStart) continue;
         if (adjEnd && adjDate > adjEnd) continue;
      ...
      return m;
   }, [filteredAssignments, feeAdjustments, timeframe, dateRange]);
               ...
               <div className="space-y-6 p-2">
                  <div className="flex items-center justify-between">
                     <UserAvatarProfile
                        user={{
                           fullName:
                              rider.fullName || rider.full_name || "Unnamed",
                           subTitle: rider.phone || rider.vehicle || "",
                           imageUrl:
                              rider.imageUrl || rider.image_url || undefined,
                        }}
                        showInfo
                     />
         ...
         >
            <AlertDialogContent>
               <AlertDialogHeader>
                  ...
                  <AlertDialogDescription>
                     Add a manual transport fee that was paid outside the
                     system. Select the transaction date to ensure it appears
                     correctly in the rider's earnings history.
                  </AlertDialogDescription>
               </AlertDialogHeader>
               <div className="space-y-4 py-4">
                  ...
                  <div className="space-y-2">
                     ...
                     <p className="text-xs text-muted-foreground">
                        When did this transaction actually occur? This affects
                        when the fee appears in earnings reports.
                     </p>
                  </div>
                  <div className="space-y-2">
                  ...
                  <AlertDialogAction
                     onClick={async () => {
                        ...
                        try {
                           ...
                           const [res, adjustments] = await Promise.all([
                              authorizedAPI.get(
                                 `/riders/${riderId}/details?limit=1000`,
                              ),
