...
const ThankYou: React.FC<ThankYouProps> = ({ message, isValidAccess = true }) => {
  const { t } = useLanguage();
  return (
      ...
      <div className="relative z-10 max-w-lg mx-4 w-full">
        <div className="bg-white/80 backdrop-blur-xl rounded-3xl shadow-2xl shadow-sky-200/50 p-10 border border-white/50">
          ...
          <h1 className="text-3xl font-bold text-center mb-3 bg-gradient-to-r from-sky-600 via-sky-500 to-orange-500 bg-clip-text text-transparent">
            {isValidAccess
              ? t("thankYou.title")
              : t("thankYou.visitorTitle") || "Thank You for Coming to NiheMart!"}
          </h1>
          ...
          <div className="text-center mb-6">
            ...
            {!isValidAccess && (
              <div className="mb-4">
                <Link
                  href="/products"
                  className="inline-block rounded-lg bg-gradient-to-r from-sky-500 to-sky-600 hover:from-sky-600 hover:to-sky-700 text-white px-6 py-3 font-semibold transition-all duration-300 hover:shadow-lg hover:scale-105"
                >
                  {t("thankYou.shopNow") || "Shop Now →"}
                </Link>
              </div>
            )}
          ...
          {/* Order status visual - only show for valid order access */}
          {isValidAccess && (
