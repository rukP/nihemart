...
export function ManageRefundDialog({ open, onOpenChange, order, item }: Props) {
    ...
    try {
      if (target === 'item') {
        ...
        if (currentItem && currentItem.refund_status === 'requested') {
          await respond.mutateAsync({ itemId, approve, _note: note });
        } else {
          ...
          await respond.mutateAsync({ itemId, approve: true, _note: note });
        }
      } else {
  ...
  // Check if this is a customer-requested refund (admin just approves/rejects)
  const isCustomerRequested =
    item?.refund_status === 'requested' || order.refund_status === 'requested';
      ...
      <DialogContent className="max-w-2xl px-1 sm:px-4">
        ...
        <ScrollArea className="max-h-[75vh]">
          {nonRefundableOrder && (
            <Card className="p-3 border-0 bg-yellow-50 shadow-none">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="font-semibold text-sm">Refund Not Allowed</p>
          ...
          <div className="space-y-4 p-2 sm:p-4">
            ...
            {/* Refund Target Selection - Only show for admin-initiated refunds */}
            {!isCustomerRequested && (
              <Card className="p-4">
                <div className="flex items-center gap-2 mb-3">
                ...
                <div className="grid grid-cols-2 gap-3 mb-4">
                  <button
                    onClick={() => !nonRefundableOrder && setTarget('order')}
                    className={cn(
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-medium text-sm">Whole Order</span>
                    </div>
                  ...
                  <button
                    onClick={() => !nonRefundableOrder && setTarget('item')}
                    className={cn(
                      'p-4 rounded-lg border-2 transition-all text-left hover:shadow-md',
                      target === 'item'
                        ? 'border-blue-500 bg-blue-50 shadow-md'
                  ...
                  >
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      <span className="font-medium text-sm">Specific Item</span>
                    </div>
                    <p className="text-xs text-muted-foreground">
                ...
                {/* Item Selection - Only show for admin-initiated refunds */}
                {/* FIXED: Filter out rejected items from selection */}
                {target === 'item' && !isCustomerRequested && (
                  <div className="space-y-2 pt-2 border-t">
                    ...
                    {(order.items || [])
                      .filter(
                        it => !it.rejected && it.refund_status !== 'rejected'
                      )
                      .map(it => {
                        ...
                        return (
                          <button
                            ...
                            disabled={nonRefundableOrder || nonRefundableItem}
                          >
            ...
            {/* Customer Requested Refund Info - Show what customer is requesting */}
            {isCustomerRequested && (
              <Card className="p-4 border-0 bg-blue-50 shadow-none">
                    ...
                    <div className="border-t border-blue-200 pt-3">
                      {item ? (
                        <div className="space-y-2">
                          ...
                          {item.refund_reason && (
                            <div className="mt-2">
                              ...
                              <p className="text-sm italic bg-white rounded p-2 border border-blue-200">
                                "{item.refund_reason}"
                              </p>
                            </div>
                          )}
                        ...
                        <div className="space-y-2">
                          ...
                          <div className="bg-white rounded-lg p-3 border border-blue-200">
                            <p className="text-sm font-medium">
                              All {order.items?.length || 0} item(s)
                            </p>
            ...
            {!isCustomerRequested && (item || order.refund_reason) && (
              <Card className="p-4 border-0 bg-amber-50 shadow-none">
                <div className="flex items-start gap-2">
                  <AlertCircle className="h-4 w-4 text-amber-600 mt-0.5 flex-shrink-0" />
                  <div className="flex-1 min-w-0">
                    <h4 className="font-semibold text-sm mb-1">
                      {item ? 'Item Refund Reason' : 'Order Refund Reason'}
                    </h4>
                    <p className="text-sm italic text-amber-900">
                      {item ? item.refund_reason : order.refund_reason}
                    </p>
                    {item && (
        ...
        <DialogFooter className="border-t pt-4">
          <div className="flex gap-2 justify-end w-full">
            ...
            {isCustomerRequested ? (
              <>
                {/* Customer-requested: show both Reject and Approve */}
                <Button
                  variant="destructive"
              ...
              /* Admin-initiated: show only Approve */
              <Button
                className="bg-green-600 hover:bg-green-700 min-w-[120px]"
                onClick={() => handleResponse(true)}
                disabled={
