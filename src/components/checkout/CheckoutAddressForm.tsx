...
type Props = {
  ...
  isLoggedIn?: boolean;
  // Callback to update temp address when editing temp address
  onUpdateTempAddress?: (addressData: any) => void;
};
...
export default function CheckoutAddressForm(props: Props) {
  ...
  const saveClicked = async () => {
    ...
    try {
      ...
      if (editingAddressId) {
        // Handle temp address editing
        if (editingAddressId === 'temp-address-edit' && onUpdateTempAddress) {
          ...
          onUpdateTempAddress(tempAddressData);
          toast.success(t('checkout.updatedSuccess') || 'Address updated');
        } else {
          // Handle saved address editing
    ...
    <div className="border border-orange-200 rounded-lg p-3 sm:p-4 bg-gradient-to-r from-orange-50 to-orange-25 space-y-3 sm:space-y-4">
            ...
            return (
              <div>
                ...
                >
                  <SelectTrigger className="border-gray-300 focus:border-orange-500 focus:ring-orange-500 text-xs sm:text-sm h-9 sm:h-10">
                    <SelectValue
                      placeholder={t('checkout.selectSectorPlaceholder')}
                    />
                  </SelectTrigger>
                  <SelectContent className="text-xs sm:text-sm max-h-48 sm:max-h-56">
                    {sectors
      ...
      <div className="pt-1 sm:pt-2 space-y-2 sm:space-y-3">
        ...
        <div className="flex flex-col sm:flex-row space-y-2 sm:space-y-0 sm:space-x-2 pt-1 sm:pt-2">
          {/* Logged-in users: Save to database and use immediately */}
          {/* Guest users: Use without saving (temporary address in localStorage) */}
          {isLoggedIn ? (
            // LOGGED-IN USERS: Always save to database
            <Button
              onClick={saveClicked}
            ...
            // GUEST USERS: Use temporarily without saving
            <Button
            ...
            // NON-CHECKOUT FLOW: Regular save
            <Button
              onClick={saveClicked}
              disabled={isSavingAddress}
              className="bg-orange-500 hover:bg-orange-600 text-white text-xs sm:text-sm h-9 sm:h-10"
