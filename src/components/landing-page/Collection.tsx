...
import type { CategoryLight } from "@/lib/api/categories";
  ...
  return (
    <MaxWidthWrapper size={"lg"} className="">
      <div className="my-5 lg:my-10 relative">
        <h1 className="lg:text-4xl md:text-2xl text-xl font-bold text-neutral-900 mb-5" suppressHydrationWarning>
          {t("home.categories")}
        </h1>
                    ...
                    >
                      {Array(4)
                        ...
                        .map((__, i) => (
                          <div
                            key={i}
                            className="w-full h-32 md:h-60 bg-gray-200 rounded-2xl"
                          />
                        ))}
                    </div>
                  ))
              : // desktop single row placeholders
                Array(8)
                  ...
                  .map((_, i) => (
                    <div
                      key={i}
                      className="w-56 md:w-60 h-60 shrink-0 aspect-[9/12] bg-gray-200 rounded-2xl"
                    />
                  ))
            : isMobile
              ? // Mobile: pages with 2 rows x 2 cols per page
                mobilePages.map((page, pageIndex) => (
                  ...
                  >
                    {page.map((category) => {
                      const iconUrl = (category as any).icon_url || category.iconUrl;
                      const hasIcon = iconUrl && iconUrl !== "/placeholder.svg";
                        ...
                        >
                          {hasIcon ? (
                            <Image
                              ...
                              unoptimized={isLocalhostUrl(iconUrl)}
                            />
                          ) : (
                            <div className="w-20 h-20 bg-gray-200 rounded-lg flex items-center justify-center mb-2 group-hover:bg-gray-300 transition-colors">
                  ...
                  return (
                    ...
                    >
                      {hasIcon ? (
                        ...
                        <div className="w-30 h-30 bg-gray-200 rounded-lg flex items-center justify-center mb-3 group-hover:bg-gray-300 transition-colors">
