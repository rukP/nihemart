  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        // Show all products on mobile, cap to MAX_FEATURED on desktop
        const visibleProds = isDesktop
          ? shuffled.slice(0, MAX_FEATURED)
          : shuffled;
        setProducts(visibleProds);
        setOffset(Math.min(shuffled.length, MAX_FEATURED));
  ...
  useEffect(() => {
    ...
    const loadFilteredProducts = async () => {
      ...
      try {
        ...
        // Show all products on mobile, cap to MAX_FEATURED on desktop
        const visibleProds = isDesktop
          ? sortedProds.slice(0, MAX_FEATURED)
          : sortedProds;
        setProducts(visibleProds);
        setOffset(Math.min(sortedProds.length, MAX_FEATURED));
