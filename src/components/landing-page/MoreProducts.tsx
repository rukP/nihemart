      ...
      <div className="absolute z-20 top-3 flex justify-between w-full px-2">
        <span className="text-white bg-orange-500 my-auto p-1.5 text-center rounded-lg text-sm font-bold">
          ...
          (product.maxPrice ?? product.price)
            ? (product.minPrice ?? product.price) ===
              (product.maxPrice ?? product.price)
              ? (product.minPrice ?? product.price).toLocaleString()
              : `${(product.minPrice ?? product.price).toLocaleString()}-${(
                  product.maxPrice ?? product.price
                ).toLocaleString()}`
      ...
      <div className="bg-gradient-to-br from-slate-100 to-slate-200 rounded-xl p-4 aspect-square">
        <Image
          ...
          unoptimized={isLocalhostUrl(
            product?.main_image_url || product?.mainImageUrl
          )}
          // priority
          // loading="eager"
        />
...
const ProductGridSkeleton = ({ count = 4 }: { count?: number }) => (
  <div className="grid grid-cols-2 min-[500px]:grid-cols-3 min-[1000px]:grid-cols-4 xl:grid-cols-5 gap-2 md:gap-5">
    {Array.from({ length: count }).map((_, index) => (
      <div
        key={index}
        className="shrink-0 aspect-[3/4] sm:aspect-[4/5] bg-gray-200 rounded-2xl"
      />
    ))}
  </div>
);
...
const MoreProducts: FC<MoreProductsProps> = ({}) => {
  ...
  const [_allFeatured, setAllFeatured] = useState<StoreProduct[]>([]);
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        // Featured (explicit flag) and latest (by created/updated date)
        const featuredAll = (all || []).filter((p: any) => p.featured);

        const latestAll = (all || []).slice().sort((a: any, b: any) => {
        ...
        const sortByStock = (products: any[]) => {
          const inStock = products.filter(p => (p.stock ?? 0) > 0);
          const outOfStock = products.filter(p => (p.stock ?? 0) <= 0);
          return [...inStock, ...outOfStock];
        };

        const featuredToShow = isDesktop
          ? featuredAll.slice(0, 15)
          : featuredAll;
        const latestToShow = isDesktop ? latestAll.slice(0, 15) : latestAll;

        setAllFeatured(
          sortByStock(featuredToShow).map((p: any) => ({
            ...p,
            status: p.status || 'active',
          }))
        );
        setLatestProducts(
          sortByStock(latestToShow).map((p: any) => ({
            ...p,
            status: p.status || 'active',
          }))
        );
  ...
  const productPairs = useMemo(() => {
    const pairs = [];
    for (let i = 0; i < latestProducts.length; i += 4) {
      pairs.push(latestProducts.slice(i, i + 4));
    }
    return pairs;
  }, [latestProducts]);
