...
const ProductGridSkeleton = ({ count = 4 }: { count?: number }) => (
  <div className="grid grid-cols-2 min-[500px]:grid-cols-3 min-[1000px]:grid-cols-4 xl:grid-cols-5 gap-1.5 md:gap-5">
    {Array.from({ length: count }).map((_, index) => (
      <div
        key={index}
        className="shrink-0 aspect-[3/3.8] sm:aspect-[4/5] bg-gray-200 rounded-xl"
      />
    ))}
  </div>
);
...
const MoreToLove: FC<MoreToLoveProps> = ({}) => {
  ...
  const allShuffledRef = useRef<StoreProduct[]>([]);

  const initialLimit = 20;
  const loadMoreLimit = 10;

  const shuffle = (arr: StoreProduct[]) => {
    const a = arr.slice();
  ...
  useEffect(() => {
    const loadInitialData = async () => {
      ...
      try {
        ...
        const slice = shuffled.slice(0, initialLimit);
        setMoreToLove(slice);
  ...
  const handleLoadMore = async () => {
    ...
    try {
      const all = await fetchAllStoreProductsCached();
      // Ensure we have a shuffled pool saved (in case initial load didn't set it)
      if (allShuffledRef.current.length === 0) {
        allShuffledRef.current = shuffle(all || []);
      }

      const newSlice = allShuffledRef.current.slice(
        offset,
