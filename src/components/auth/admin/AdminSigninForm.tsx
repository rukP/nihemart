...
const AdminSigninForm: FC<AdminSigninFormProps> = ({ redirect }) => {
  ...
  const searchParams = useSearchParams(); // Hook must be called at top level

  // Use the prop if provided, otherwise derive from searchParams
  const redirectParam = redirect ?? searchParams?.get('redirect') ?? null;
