              ...
              <div key={item.id} className="border rounded p-3">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right text-sm text-gray-500">
                    ...
                    <div>{item.user?.fullName || 'you'}</div>
                  </div>
                </div>
              </div>
            ))}
            ...
            {history.map((item: StockHistoryItem) => (
              <div key={item.id} className="border rounded p-3">
                <div className="flex justify-between items-start">
                  ...
                  <div className="text-right text-sm text-gray-500">
                    <div>{new Date(item.createdAt).toLocaleDateString()}</div>
                    {/* <div>{item.user?.fullName || 'Unknown user'}</div> */}
                    <div>{item.user?.fullName || 'You'}</div>
                  </div>
                </div>
              </div>
      ...
      if (product.variations.length > 0) {
        product.variations.forEach(variation => {
          ...
          items.push({
            ...
            mainImageUrl: optimizeImageUrl(product.mainImageUrl ?? null, {
              width: 80,
              quality: 75,
            }),
          });
...
export default function StockLevels() {
  ...
  const _inStockProducts = products.filter(p =>
    p.variations.some(v => v.stock > 0)
  );

  const _outOfStockProducts = products.filter(p =>
