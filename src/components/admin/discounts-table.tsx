      ...
      <Table>
        ...
        <TableBody>
          {loading ? (
            ...
            <TableRow>
              <TableCell colSpan={9} className="h-24 text-center">No discounts found.</TableCell>
            </TableRow>
          ) : (
            discounts.map((discount) => (
              <TableRow key={discount.id}>
                ...
                <TableCell>
                  <span className="text-sm">{discount.productCount || 0} products</span>
                </TableCell>
                <TableCell>
                  <div className="text-xs text-gray-600">
                    {discount.startDate && (
                      <div>Start: {format(new Date(discount.startDate), 'MMM dd, yyyy')}</div>
                    )}
                    {discount.endDate && (
                      <div>End: {format(new Date(discount.endDate), 'MMM dd, yyyy')}</div>
                    )}
                ...
                <TableCell className="text-right">
                  <div className="flex justify-end gap-2">
                    <Button variant="ghost" size="icon" onClick={() => onView(discount)}>
                      <Eye className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => onEdit(discount)}>
                      <Edit className="h-4 w-4" />
                    </Button>
                    <Button variant="ghost" size="icon" onClick={() => onDelete(discount.id)}>
