    ...
    switch (error.code) {
      case '23505':
        if (error.message?.includes('products_sku_key')) {
          errorMessage =
            'Product SKU already exists. Please choose a different SKU.';
        } else if (error.message?.includes('product_variations_sku_key')) {
          errorMessage =
            'Variation SKU already exists. Please choose a different SKU for the variation.';
        } else {
          errorMessage =
            'Duplicate value error. Please check for unique fields.';
        }
        break;
      case '23503':
        errorMessage =
          'Invalid reference. Please check category or subcategory.';
        break;
      case 'InvalidJWT':
      ...
      case 'ResourceAlreadyExists':
        errorMessage =
          'Resource already exists. Use upsert or choose different name.';
        break;
      case 'InvalidKey':
