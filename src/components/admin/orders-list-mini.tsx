...
import { createColumns } from "../orders/columns";
...
const OrdersListMini: React.FC = () => {
   ...
   // Fetch assignments in batch for all orders
   const orderIds = orders.map((order: Order) => order.id);
   const { data: assignmentsMap = {} } = useOrderAssignmentsBatch(orderIds, !isLoading && orders.length > 0);
   
   // Create columns with assignments map
