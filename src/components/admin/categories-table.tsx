          ...
          {loading ? (
            ...
            categories.map((category) => (
              <TableRow key={category.id}>
                <TableCell>
                  {(((category as any).icon_url || category.iconUrl) || category.iconUrl) ? (
                    <div className="w-10 h-10 relative">
                        <Image 
                          src={optimizeImageUrl(((category as any).icon_url || category.iconUrl) || category.iconUrl, { width: 40, quality: 75 })} 
                          alt={category.name} 
                          fill 
                ...
                <TableCell className="text-right">{(category as any).products_count || 0}</TableCell>
                <TableCell className="text-right">{(category as any).subcategories_count ?? category.subcategories_count ?? 0}</TableCell>
