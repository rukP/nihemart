...
export default function StockOverview({
  ...
  const { data: _orderStats } = useOrderStats();
  ...
  const metrics = (() => {
    ...
    products.forEach(product => {
      if (product.variations.length > 0) {
        ...
        lowStockVariations += product.variations.filter(
          v => v.stock > 0 && v.stock <= 10
        ).length;
        outOfStockVariations += product.variations.filter(
          v => v.stock <= 0
        ).length;
      } else {
    ...
    const totalStockValue = products.reduce((total, product) => {
      if (product.variations.length > 0) {
        return (
          total +
          product.variations.reduce((varTotal, variation) => {
  ...
  const stockChartData = [
    {
      month: 'Current',
      inStock: metrics.inStockVariations,
      lowStock: metrics.lowStockVariations,
      outOfStock: metrics.outOfStockVariations,
    },
  ];
  ...
  const ordersChartData = useMemo(() => {
    ...
    const fulfillmentCounts: Record<
      string,
        ...
        <Card className="bg-white border-l-4 border-l-orange-500">
          ...
          <CardContent>
            ...
            <p className="text-xs text-gray-500 mt-1">
              {metrics.totalVariations} variations
            </p>
          </CardContent>
        </Card>
        ...
        <Card className="bg-white border-l-4 border-l-green-500">
          ...
          <CardContent>
            <div className="text-2xl font-bold text-green-600">
              {metrics.inStockVariations}
            </div>
            <p className="text-xs text-gray-500 mt-1">
              {metrics.totalVariations > 0
                ? Math.round(
                    (metrics.inStockVariations / metrics.totalVariations) * 100
                  )
                : 0}
              % available
            </p>
        ...
        <Card className="bg-white">
          ...
          <CardContent>
            <ChartContainer config={stockChartConfig}>
              <BarChart accessibilityLayer data={stockChartData} height={200}>
