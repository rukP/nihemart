  ...
  () => {
    ...
    for (const [k, v] of _rateStore.entries()) {
      ...
      const limit = k.startsWith('email:')
        ? RATECONFIG.email.windowMs
        : RATECONFIG.ip.windowMs;
...
export async function POST(req: NextRequest) {
  try {
    ...
    const ipCheck = isRateLimited(ipKey, RATECONFIG.ip);
    if (ipCheck.limited) {
      return new Response(
        JSON.stringify({
          ok: false,
    ...
    const emailCheck = isRateLimited(emailKey, RATECONFIG.email);
    ...
    const to =
      ...
      'support@nihemart.rw';
    const _from = process.env.EMAILFROM || `no-reply@nihemart.rw`;
    ...
    const logoUrl =
      ...
      (process.env.NEXT_PUBLIC_APP_URL || 'https://nihemart.rw').replace(
        /\/$/,
        ''
      ) + '/logo.png';
