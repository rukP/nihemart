  ...
  try {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        {
          error:
            backendData.message || backendData.error || 'Payment not found',
        },
        { status: backendResponse.status }
      );
    }
    ...
    if (isReference && backendData.session) {
      ...
      transformed = {
        ...
        kpay_transaction_id:
          payment.kpayTransactionId || payment.kpay_transaction_id,
        ...
        kpay_mom_transaction_id:
          payment.kpayMomTransactionId || payment.kpay_mom_transaction_id,
        ...
        client_timeout_reason:
          payment.clientTimeoutReason || payment.client_timeout_reason,
...
export async function PATCH(request: NextRequest, { params }: RouteParams) {
  try {
    ...
    logger.warn(
      'api',
      'PATCH payment endpoint called - not needed in one-way flow',
      {
        paymentId,
        orderId: order_id,
      }
