  ...
  try {
    ...
    const backendResponse = await fetch(backendUrl, {
      ...
      body: JSON.stringify({
        ...
        paymentMethod: body.paymentMethod,
        redirectUrl:
          body.redirectUrl || `${appBaseUrl}/payment/${body.orderId}`,
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            backendData.message || backendData.error || 'Payment retry failed',
        },
        { status: backendResponse.status }
      );
    }
    ...
    const checkoutUrl =
      kpayData.url || kpayData.redirecturl || kpayData.redirectUrl;
