   ...
   try {
      ...
      const transformedPayments = payments.map((p: any) => ({
         ...
         client_timeout_reason:
            p.clientTimeoutReason || p.client_timeout_reason,
         ...
         order: p.order
            ? {
                 ...
                 customer_email:
                    p.order.customerEmail || p.order.customer_email,
                 customer_phone:
                    p.order.customerPhone || p.order.customer_phone,
                 total: p.order.total,
                 payment_status:
                    p.order.paymentStatus || p.order.payment_status,
                 delivery_address:
                    p.order.deliveryAddress || p.order.delivery_address,
                 delivery_city: p.order.deliveryCity || p.order.delivery_city,
                 delivery_notes:
                    p.order.deliveryNotes || p.order.delivery_notes,
                 user_id: p.order.userId || p.order.user_id,
                 is_external:
                    p.order.isExternal || p.order.is_external || false,
