...
export async function GET(request: NextRequest) {
  try {
    ...
    const productsResponse = await fetch(`${API_BASE}/products?page=1&limit=1000&status=all`, {
      headers: {
    ...
    for (const product of products) {
      try {
        const reviewResponse = await fetch(`${API_BASE}/products/${product.id}/reviews`, {
          headers: {
            'Content-Type': 'application/json',
            ...(token ? { Authorization: `Bearer ${token}` } : {}),
          },
