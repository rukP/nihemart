    ...
    <CardContent className="space-y-4 sm:space-y-6">
      <div className="flex items-start gap-3 sm:gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-gray-500 text-xs sm:text-sm truncate">ID: #{id}</p>
          <Badge
...
const Dashboard = () => {
  ...
  const [_tempTopRange, _setTempTopRange] = useState<{
    from?: string;
    to?: string;
  }>({});
  ...
  const { data: rider, isLoading: _loadingRider } = useMyRiderProfile();
  ...
  const getDateRangeForTimeframe = (tf: string): { start: Date; end: Date } => {
    const now = new Date();
  ...
  const _accepted = currentStats.accepted;
  const _rejected = currentStats.rejected;
  ...
  const _acceptedChange = calculateChange(
    currentStats.accepted,
    previousStats.accepted,
    topIsCustom
  );
  const _rejectedChange = calculateChange(
  ...
  const _analyticsTotalChange = calculateChange(
  ...
  const _analyticsPendingChange = calculateChange(
  ...
  const assignmentDeliveries = filteredAssignments.slice(0, 5).map((a: any) => {
    const order =
      a.order ||
      a.orders ||
  ...
  const _hourlyDeliveriesData = React.useMemo(() => {
          ...
          <div className="mb-6 sm:mb-8">
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900 mb-2">
              {t('rider.welcomeBack').replace(
                '{name}',
                rider?.fullName || user?.fullName || user?.email || 'Rider'
              )}
            </h1>
            <p className="text-sm sm:text-base text-gray-600">
              {t('rider.overview')}
                    ...
                    <div className="flex items-center gap-3">
                      <div className="w-44">
                        <Select value={timeframe} onValueChange={setTimeframe}>
                          <SelectTrigger>
                            <SelectValue placeholder={t('rider.last7days')} />
                          </SelectTrigger>
                          <SelectContent>
                          ...
                          <div className="space-y-2 w-64">
                            <div>
                              ...
                              <input
                                ...
                                className="w-full rounded-md border px-3 py-2"
                                placeholder="From date"
                                value={tempAnalyticsRange.from || ''}
                                onChange={e =>
                                  setTempAnalyticsRange(prev => ({
                                    ...prev,
                                    from: e.target.value || undefined,
                                  }))
                                }
                        ...
                        <div className="flex items-center justify-between">
                          <div className="space-y-1">
                            <p className="text-gray-500 text-xs sm:text-sm">
                              {timeframe === 'today'
                                ? t('rider.todayTotal') || 'Today Total'
                                : timeframe === 'yesterday'
                            ...
                            <p className="text-xl sm:text-2xl font-bold text-gray-900">
                              RWF {selectedDateTotal.toLocaleString()}
                            </p>
                          </div>
