...
export default function StockHistoryPage() {
  ...
  const metrics = useMemo(() => {
    ...
    const totalReduced = Math.abs(
      history.filter(h => h.change < 0).reduce((sum, h) => sum + h.change, 0)
    );
              ...
              <CardContent>
                <div
                  className={cn('text-2xl font-bold', {
                    'text-green-600':
                      metrics.totalAdded - metrics.totalReduced > 0,
                    'text-red-600':
                      metrics.totalAdded - metrics.totalReduced < 0,
                    'text-gray-600':
                      metrics.totalAdded - metrics.totalReduced === 0,
                ...
                >
                  {metrics.totalAdded - metrics.totalReduced > 0 ? '+' : ''}
                  {metrics.totalAdded - metrics.totalReduced}
                </div>
            ...
            <Card className="bg-white border-purple-200">
              ...
              <CardContent>
                <div className="text-2xl font-bold text-purple-600">
                  {
                    new Set(history.map(h => h.product?.id).filter(Boolean))
                      .size
                  }
                </div>
