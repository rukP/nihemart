...
const RidersPage = () => {
  ...
  const [_latestAssignmentsMap, setLatestAssignmentsMap] = useState<
    Record<string, any>
  >({});
  ...
  // Use 365 days (all-time) to show actual earnings instead of just last 7 days
  useEffect(() => {
    (async () => {
      try {
        const { riderAPI } = await import('@/hooks/useRiders');
