...
import { ShoppingCart, Users, Download, Bike } from 'lucide-react';
...
const DashboardContent: React.FC = () => {
  ...
  const { users: _allUsers, loading: _allUsersLoading } = useUsers();
  ...
  const { data: totalUsersCount, isLoading: _usersCountLoading } = useQuery({
  ...
  const { data: earningsResponse, isLoading: _earningsLoading } = useQuery({
    queryKey: [
  ...
  const orders = Array.isArray(ordersResponse)
    ? ordersResponse
    : ordersResponse?.data || [];
  const _products = productsResponse || [];
  ...
  const _filteredProducts = topProducts.slice(0, 5);
