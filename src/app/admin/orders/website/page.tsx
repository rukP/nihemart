...
export default function WebsiteOrdersPage() {
  ...
  const hasMore =
    ...
    (orders.length === limit &&
      (allWebsiteOrdersResponse?.count ?? ordersResponse?.count ?? 0) === 0);
  ...
  const ordersData: WebsiteOrderRow[] = (ordersResponse?.data || [])
    .map((o: Order) => ({
      id: o.id,
                  ...
                  <DropdownMenuItem
                    onClick={async () => {
                      try {
                        ...
                        console.error("Refund by admin failed:", err);
                      }
                    }}
                  >
                    Refund By Admin
                  ...
                  <DropdownMenuItem
                    onClick={async () => {
                      try {
                        ...
                        console.error("Label order failed:", err);
                      }
                    }}
                  >
                    Label Order
  ...
  return (
    <ScrollArea className="bg-surface-secondary h-[calc(100vh-5rem)]">
      <div className="px-5 sm:px-10 py-10">
        ...
        <div className="mb-6">
          {(() => {
            const allOrders = (allWebsiteOrdersResponse?.data || []) as Order[];
            const total =
              allWebsiteOrdersResponse?.pagination?.total ??
              allWebsiteOrdersResponse?.count ??
              allOrders.length;
