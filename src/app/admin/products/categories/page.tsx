...
export default function CategoriesPage() {
  ...
  const loadCategories = useCallback(async () => {
    ...
    try {
      ...
      const categories = Array.isArray(result) ? result : (result as any)?.data || result || [];
  ...
  const loadCategorySubcategories = useCallback(async (categoryId: string) => {
    try {
      console.log("[CategoriesPage] Loading subcategories for category:", categoryId);
  ...
  return (
    <div className="flex flex-col space-y-6 p-6">
      ...
      <AddEditSubcategoryDialog
        ...
        categories={categories}
        defaultCategoryId={viewingCategory?.id} // FIXED: Pre-select viewing category when adding subcategory
      />
    </div>
  );
}
