...
function ProductsPageContent() {
  ...
  useEffect(() => {
    const loadCategoriesData = async () => {
      try {
        const resp: any = await fetchCategoriesWithSubcategories();
        // Normalize response shape
        let categoriesWithSubs: any[] = [];
        if (Array.isArray(resp)) {
          categoriesWithSubs = resp;
        } else if (resp && Array.isArray(resp.categories)) {
          categoriesWithSubs = resp.categories;
        ...
        const allSubcategories = categoriesWithSubs.flatMap((c: any) =>
