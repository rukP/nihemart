...
import { Shield, Users, CheckCircle2, XCircle, Search, Edit2, Trash2 } from "lucide-react";
import { toast } from "sonner";
import userAPI, { type AppRole, type UserRow } from "@/lib/api/users";
...
const ALL_ROLES: AppRole[] = ["admin", "manager", "stock_manager", "staff", "rider", "user"];
...
const UserRolesPermissionsPage: FC = () => {
  const { users, loading, updateUserRole, deleteUser, roleCounts } = useUsers();
  ...
  const filteredUsers = useMemo(() => {
    ...
    return users.filter(
      (user) => {
        const fullName = ((user as any).full_name || user.fullName || "").toLowerCase();
        return (
          user.email.toLowerCase().includes(query) ||
          fullName.includes(query) ||
          user.roles?.some((r) => ROLE_LABELS[r].toLowerCase().includes(query))
        );
      }
    );
  }, [users, searchQuery]);
        ...
        <div className="flex min-w-0 flex-col px-2 py-10 xs:px-5 sm:px-10">
          ...
          <div className="mb-6">
            ...
            <p className="text-muted-foreground">
              Manage user roles and their access permissions across the admin dashboard
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
                    ...
                    <TableBody>
                      {ADMIN_SECTIONS.map((section) => (
                        <TableRow key={section}>
                          ...
                          {ALL_ROLES.map((role) => {
                            const hasAccess = roleAccessMap[section].includes(role);
                            return (
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div>
                  <Badge className={ROLE_COLORS.admin} variant="outline">
                    Admin
                  </Badge>
                ...
                <div>
                  <Badge className={ROLE_COLORS.manager} variant="outline">
                    Manager
                  </Badge>
                  <p className="text-sm text-muted-foreground mt-2">
                ...
                <div>
                  <Badge className={ROLE_COLORS.stock_manager} variant="outline">
                    Stock Manager
                  </Badge>
                  <p className="text-sm text-muted-foreground mt-2">
                ...
                <div>
                  <Badge className={ROLE_COLORS.staff} variant="outline">
                    Staff
                  </Badge>
                  <p className="text-sm text-muted-foreground mt-2">
                ...
                <div>
                  <Badge className={ROLE_COLORS.rider} variant="outline">
                    Rider
                  </Badge>
                  <p className="text-sm text-muted-foreground mt-2">
                ...
                <div>
                  <Badge className={ROLE_COLORS.user} variant="outline">
                    User
                  </Badge>
                  <p className="text-sm text-muted-foreground mt-2">
                  ...
                  <Table>
                    ...
                    <TableBody>
                      {filteredUsers.map((user) => {
                        const fullName = (user as any).full_name || user.fullName || "N/A";
                        const createdAt = (user as any).created_at || user.createdAt;
                        return (
