...
const AddressesPage = () => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const first =
        ve?.errors?.[0]?.message || 'Please enter a valid phone number';
    ...
    try {
      ...
      const derivedDisplayName =
        displayName ||
        (sectorObj ? `${sectorObj.sct_name} address` : 'Address');
  ...
  const handleSetDefault = async (id: string) => {
    if (!isLoggedIn) {
      toast.error('Please sign in to set a default address');
      return;
    }
                      ...
                      <div>
                        ...
                        >
                          ...
                          <SelectContent className="text-xs sm:text-sm max-h-48 sm:max-h-56">
                            {sectors
                              .filter(
                                (s: any) => s.sct_district === selectedDistrict
                              )
                              .map((s: any) => (
                                <SelectItem
                                  key={s.sct_id}
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="space-y-1 sm:space-y-2">
                      <p className="text-xs sm:text-sm text-gray-600 flex items-start sm:items-center break-words">
                        ...
                        {[addr.house_number, addr.street, addr.city]
                          .filter(Boolean)
                  ...
                  <div className="flex flex-wrap gap-2 sm:flex-nowrap sm:flex-col sm:items-end">
                    ...
                    <div className="flex gap-2 order-1 sm:order-2">
                      <Button
                        ...
                        className="border-orange-300 text-orange-600 hover:bg-orange-50 text-xs h-7 sm:h-8 px-2 whitespace-nowrap"
                      >
                        {selected?.id === addr.id ? 'Unselect' : 'Select'}
                      </Button>
                      {!addr.is_default && isLoggedIn && (
                        <Button
                          size="sm"
                          variant="outline"
