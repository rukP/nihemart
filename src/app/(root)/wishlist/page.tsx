              ...
              <CardContent className="p-0">
                  ...
                  <div className="relative aspect-square bg-gray-100 rounded-md overflow-hidden p-2 sm:p-3">
                    <Image
                      src={optimizeImageUrl(
                        ((item.product as any)?.main_image_url || item.product?.mainImageUrl) || "/placeholder.svg",
                        {
                          width: 900,
                          quality: 80,
                        }
                ...
                <div className="p-3 sm:p-4">
                  ...
                  <p className="text-gray-600 text-sm mb-2 line-clamp-1 sm:line-clamp-2">
                    {((item.product as any)?.short_description || (item.product as any)?.shortDescription) || ""}
                  </p>

                  <div className="flex items-center justify-between mb-3">
                    <span className="text-lg sm:text-xl font-bold text-orange-600">
                      {t("wishlist.price")}{" "}
                      {((item.product as any)?.minPrice ?? item.product?.price) &&
                      ((item.product as any)?.maxPrice ?? item.product?.price)
                        ? ((item.product as any)?.minPrice ?? item.product?.price) ===
                          ((item.product as any)?.maxPrice ?? item.product?.price)
                          ? ((item.product as any)?.minPrice ?? item.product?.price).toLocaleString()
                          : `${((item.product as any)?.minPrice ?? item.product?.price).toLocaleString()}-${((item.product as any)?.maxPrice ?? item.product?.price).toLocaleString()}`
