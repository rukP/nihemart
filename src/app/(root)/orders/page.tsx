...
const Orders = () => {
  ...
  // Normalize orders data to handle both PaginatedOrdersResponse and Order[] types
  const normalizedOrdersData = ordersData
    ? Array.isArray(ordersData)
      ? { data: ordersData, count: ordersData.length }
      : { ...ordersData, count: ordersData.count || ordersData.data.length }
    : null;
  ...
  return (
    <div className="container mx-auto px-3 sm:px-4 py-6 sm:py-8 max-w-[90vw]">
      ...
      {isLoading ? (
      ...
      ) : !normalizedOrdersData?.data ||
        normalizedOrdersData.data.length === 0 ? (
        ...
        <>
          <div className="space-y-4 sm:space-y-6">
            {normalizedOrdersData.data.map((order: Order) => (
              <Card
                key={order.id}
                        ...
                        .map((item: OrderItem, index: number) => (
                          ...
                          >
                            <div className="min-w-0 flex-1 pr-4">
                              ...
                              {/* FIXED: Make variant display more prominent in order list */}
                              {(item.variation_name ||
                                ...
                                <div className="mt-1">
                                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-xs font-medium bg-blue-100 text-blue-800 border border-blue-200">
                                    {item.variation_name ||
                                      `Variant ID: ${item.product_variation_id?.slice(0, 8)}...`}
                                  </span>
                                </div>
                              )}
                            ...
                            <p className="font-medium text-sm sm:text-base whitespace-nowrap">
                              {Number(item.total).toLocaleString()} RWF
                            </p>
                          </div>
                        ))
                    ) : (
                    ...
                    <div className="text-xs sm:text-sm text-muted-foreground">
                      ...
                      {order.status === 'delivered' &&
                        ...
                        (() => {
                          ...
                          const within24h =
                            Date.now() - deliveredAt <= 24 * 60 * 60 * 1000;
                          if (!within24h) {
                            return (
                              <div className="text-gray-500 mt-1 text-xs">
                                ⏰ Refund window expired (24 hours after
                                delivery)
                              </div>
                            );
                          }
          ...
          <Dialog open={refundDialogOpen} onOpenChange={setRefundDialogOpen}>
            <DialogContent className="max-w-md">
              <DialogHeader>
                ...
                <DialogDescription>
                  Please provide a reason for requesting a full refund for this
                  order.
                </DialogDescription>
              </DialogHeader>
              <div className="mt-4">
          ...
          {/* Pagination */}
          {normalizedOrdersData && normalizedOrdersData.count > 10 && (
            <div className="flex flex-col sm:flex-row justify-center items-center gap-2 mt-6 sm:mt-8">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs sm:text-sm text-muted-foreground px-3">
                  ...
                  {Math.ceil(normalizedOrdersData.count / 10)}
                </span>
                <Button
                  variant="outline"
                  onClick={() => handlePageChange(filters.page + 1)}
                  disabled={
                    filters.page >= Math.ceil(normalizedOrdersData.count / 10)
                  }
                  className="text-sm h-9"
                >
                  Next
