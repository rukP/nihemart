...
function ProductListingComponent() {
  ...
  const fetchProducts = useCallback(async () => {
    ...
    try {
      ...
      const { data, count } = await fetchStoreProducts({
        ...
        pagination: { page: filters.page, limit: PAGELIMIT },
      });
      // Sort products: in-stock first, then out-of-stock (better UX)
      const sortedData = (() => {
        const inStock = data.filter((p: any) => (p.stock ?? 0) > 0);
        const outOfStock = data.filter((p: any) => (p.stock ?? 0) <= 0);
        return [...inStock, ...outOfStock];
      })();
      setProducts(
        sortedData.map((p: any) => ({ ...p, status: p.status || 'active' }))
      );
      setTotalCount(count);
    } catch (_error) {
  ...
  const _renderStars = (rating: number | null) => {
        ...
        {expandedSections.category && (
          <div className="space-y-1">
            {categories.map(c => {
              ...
              const isExpanded = expandedCategories.indexOf(c.id) !== -1;
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-3 xl:grid-cols-4 gap-2 md:gap-5 mb-8">
                {loading
                  ? Array.from({ length: PAGELIMIT }).map((_, i) => (
                      <Card key={i} className="bg-gray-200 h-80 border-0">
                            ...
                            <div className="absolute top-2 right-2">
                              ...
                              0 ? (
                                <Button
                                  ...
                                  aria-label="Add to wishlist - product out of stock"
                                >
