// Rider info for order assignment/delivery
export interface Rider {
  id: string;
  full_name: string;
  email?: string;
  phone?: string;
  imageUrl?: string;
  vehicle?: string;
  location?: string;
  // Added vehicle and location fields
}
export type OrderStatus =
  | 'pending'
  | 'processing'
  | 'shipped'
  | 'assigned'
  | 'delivered'
  | 'cancelled'
  | 'refunded';

export interface OrderBase {
  user_id?: string;
  status: OrderStatus;
  subtotal: number;
  tax?: number;
  /** Payment method identifier, e.g. 'cash_on_delivery' */
  payment_method?: string;
  total: number;
  currency?: string;
  customer_email: string;
  customer_first_name: string;
  customer_last_name: string;
  customer_phone?: string;
  delivery_address: string;
  delivery_city: string;
  delivery_notes?: string;
  /** Optional free-text notes specifically for scheduled deliveries */
  schedule_notes?: string | null;
  /** Optional scheduled delivery time requested by customer (ISO string) */
  delivery_time?: string | null;
  source?: string;
  is_external?: boolean;
  is_paid?: boolean;
}

export interface Order extends OrderBase {
  id: string;
  order_number: string;
  created_at: string;
  updated_at: string;
  shipped_at?: string | null;
  delivered_at?: string | null;
  items?: OrderItem[];
  status: OrderStatus;
  // Order-level refund fields (for full-order refunds)
  refund_requested?: boolean;
  refund_reason?: string | null;
  refund_status?: RefundStatus | null;
  refund_requested_at?: string | null;
  // Runtime marker added by server to indicate whether this was a refund or reject flow
  _mode?: 'refund' | 'reject';
  // Rider assigned or delivered this order
  rider?: Rider | null;
  // Order labeling fields
  label_number?: string | null;
  location_code?: string | null;
  is_labeled?: boolean;
  order_count?: number | null;
}

export interface OrderItem {
  id: string;
  order_id: string;
  product_id?: string | null;
  product_variation_id?: string | null;
  product_name: string;
  product_sku?: string | null;
  variation_name?: string | null;
  price: number;
  quantity: number;
  total: number;
  created_at: string;
  // Product image URL at time of order (for displaying in order history)
  product_image_url?: string | null;
  // Rejection fields (for non-delivered orders - immediate action)
  rejected?: boolean;
  rejection_reason?: string | null;
  rejected_at?: string | null;
  // Refund-related fields (for delivered orders - requires approval)
  refund_requested?: boolean;
  refund_reason?: string | null;
  // refund_status: null | 'requested' | 'approved' | 'rejected' | 'cancelled'
  refund_status?: RefundStatus | null;
  refund_requested_at?: string | null;
  // Runtime marker added by server to indicate whether this was a refund or reject flow
  _mode?: 'refund' | 'reject';
}

export interface OrderItemInput {
  product_name: string;
  quantity: number;
  price: number;
  product_id: string;
  variation_name?: string;
  product_sku?: string;
  product_variation_id?: string;
  product_image_url?: string | null;
  total: number;
}

export type RefundStatus =
  | 'requested'
  | 'approved'
  | 'rejected'
  | 'cancelled'
  | 'refunded';

export interface CreateOrderRequest {
  order: OrderBase;
  items: OrderItemInput[];
}

export interface OrderFilters {
  status?: OrderStatus;
  search?: string;
  dateFrom?: string;
  dateTo?: string;
  priceMin?: number;
  priceMax?: number;
  city?: string;
  isPaid?: boolean;
  isExternal?: boolean;
  /** Payment method identifier, e.g. 'cash_on_delivery' */
  payment_method?: string;
}

export interface OrderQueryOptions {
  filters?: OrderFilters;
  pagination?: {
    page: number;
    limit: number;
  };
  sort?: {
    column: string;
    direction: 'asc' | 'desc';
  };
}

// Backend order response (may use camelCase)
export interface BackendOrder {
  id?: string;
  orderNumber?: string;
  order_number?: string;
  userId?: string;
  user_id?: string;
  status?: OrderStatus | string;
  subtotal?: number;
  tax?: number;
  total?: number;
  currency?: string;
  customerEmail?: string;
  customer_email?: string;
  customerFirstName?: string;
  customer_first_name?: string;
  customerLastName?: string;
  customer_last_name?: string;
  customerPhone?: string;
  customer_phone?: string;
  deliveryAddress?: string;
  delivery_address?: string;
  deliveryCity?: string;
  delivery_city?: string;
  deliveryNotes?: string;
  delivery_notes?: string;
  scheduleNotes?: string;
  schedule_notes?: string;
  deliveryTime?: string | Date;
  delivery_time?: string;
  paymentMethod?: string;
  payment_method?: string;
  isPaid?: boolean;
  is_paid?: boolean;
  isExternal?: boolean;
  is_external?: boolean;
  createdAt?: string | Date;
  created_at?: string;
  updatedAt?: string | Date;
  updated_at?: string;
  shippedAt?: string | Date | null;
  shipped_at?: string | null;
  deliveredAt?: string | Date | null;
  delivered_at?: string | null;
  refundRequested?: boolean;
  refund_requested?: boolean;
  refundReason?: string | null;
  refund_reason?: string | null;
  refundStatus?: RefundStatus | null;
  refund_status?: RefundStatus | null;
  refundRequestedAt?: string | Date | null;
  refund_requested_at?: string | null;
  labelNumber?: string | null;
  label_number?: string | null;
  locationCode?: string | null;
  location_code?: string | null;
  isLabeled?: boolean;
  is_labeled?: boolean;
  items?: BackendOrderItem[];
  rider?: Rider | null;
  assignments?: OrderAssignment[];
}

export interface BackendOrderItem {
  id?: string;
  orderId?: string;
  order_id?: string;
  productId?: string | null;
  product_id?: string | null;
  productVariationId?: string | null;
  product_variation_id?: string | null;
  productName?: string;
  product_name?: string;
  productSku?: string | null;
  product_sku?: string | null;
  variationName?: string | null;
  variation_name?: string | null;
  price?: number;
  quantity?: number;
  total?: number;
  productImageUrl?: string | null;
  product_image_url?: string | null;
  createdAt?: string | Date;
  created_at?: string;
  refundRequested?: boolean;
  refund_requested?: boolean;
  refundReason?: string | null;
  refund_reason?: string | null;
  refundStatus?: RefundStatus | null;
  refund_status?: RefundStatus | null;
  refundRequestedAt?: string | Date | null;
  refund_requested_at?: string | null;
  rejected?: boolean;
  rejection_reason?: string | null;
  rejected_at?: string | null;
}

export interface OrderAssignment {
  id: string;
  orderId: string;
  riderId: string;
  assignedAt: string;
  status: string;
  rider?: Rider;
}

export interface PaginatedOrdersResponse {
  data: Order[];
  pagination?: {
    page: number;
    limit: number;
    total: number;
    totalPages: number;
  };
  count?: number;
}

export type OrderListResponse = PaginatedOrdersResponse | Order[];
