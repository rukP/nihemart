...
export function useAddresses() {
   ...
   const EXPLICIT_UNSELECT_KEY = "nihemart_explicit_unselect_address_v1";
   ...
   const fetchAddresses = useCallback(async () => {
      try {
         ...
         if (user) {
            ...
            const explicitUnselect = typeof window !== "undefined" 
               ? localStorage.getItem(EXPLICIT_UNSELECT_KEY) === "true"
               : false;
            ...
            setSelected((currentSelected) => {
               ...
               if (data && data.length > 0 && !explicitUnselect && !hasAutoSelectedOnceRef.current && !currentSelected) {
               ...
               } else if (hasAutoSelectedOnceRef.current && !explicitUnselect && currentSelected) {
                  ...
                  const updatedSelected = data?.find((addr: Address) => addr.id === currentSelected.id);
                  if (updatedSelected) {
         ...
         if (typeof window !== "undefined") {
            try {
               ...
               const explicitUnselect = localStorage.getItem(EXPLICIT_UNSELECT_KEY) === "true";
               ...
               if (raw) {
                  ...
                  setSelected((currentSelected) => {
                     ...
                     if (arr.length > 0 && !explicitUnselect && !hasAutoSelectedOnceRef.current && !currentSelected) {
   ...
   const saveAddress = async (
      ...
      if (user) {
         ...
         try {
            ...
            if (typeof window !== "undefined") {
               localStorage.removeItem(EXPLICIT_UNSELECT_KEY);
            }
   ...
   const updateAddress = async (id: string, updates: Partial<Address>): Promise<Address | null> => {
   ...
   const setDefaultAddress = async (id: string) => {
      ...
      try {
         ...
         const newDefault = updatedAddresses?.find((addr: Address) => addr.id === id);
         if (newDefault) {
            setSelected({ ...newDefault });
            hasAutoSelectedOnceRef.current = true;
         } else {
            // If the address was deleted, clear selection
            const stillExists = updatedAddresses?.find((addr: Address) => addr.id === id);
            if (!stillExists) {
               setSelected(null);
            }
         }
   ...
   const selectAddress = (id: string | null) => {
      if (!id) {
         ...
         setSelected(null);
         if (typeof window !== "undefined") {
            localStorage.setItem(EXPLICIT_UNSELECT_KEY, "true");
         }
         return;
      }
