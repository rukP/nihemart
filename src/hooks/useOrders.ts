...
function transformOrder(order: any): Order {
   ...
   return {
      ...
      refund_requested_at: order.refundRequestedAt
      ...
      items: order.items
         ? order.items.map((item: any) => ({
              ...
              product_variation_id:
                 item.productVariationId || item.product_variation_id,
              ...
              // FIXED: Ensure refund fields are properly transformed (critical for rider view)
              refund_requested:
              ...
              refund_requested_at: item.refundRequestedAt
                 ? typeof item.refundRequestedAt === "string"
                    ...
                    : item.refundRequestedAt.toISOString()
...
const internalOrderAPI = {
   ...
   requestItemRefund: async (itemId: string, reason: string): Promise<any> => {
...
export function useOrderAssignment(orderId?: string, enabled = true) {
   ...
   return useQuery({
      ...
      queryFn: async () => {
         ...
         // The backend returns assignments, but TypeScript Order type doesn't include it
      ...
      onMutate: async ({
         ...
         for (const [key, data] of previousDetails) {
            try {
               ...
               console.error("Error during optimistic update:", e);
            }
         }
         ...
         for (const [key, data] of previousLists) {
            try {
               ...
               console.error("Error during optimistic list update:", e);
            }
         }
            ...
            try {
               ...
               if (order && Array.isArray(order.items)) {
                  ...
                  if (idx !== -1) {
                     ...
                     queryClient.setQueryData(key, updated);
                     console.log(
                        "[useRejectOrderItem] Updated order detail cache",
                     );
                  }
               }
            } catch (e) {
