...
function verifyConfiguration() {
  ...
  log(
    `  Base URL (sandbox): ${config.baseUrl || 'https://pay.esicia.com (default)'}`,
    'cyan'
  );
  log(
    `  Live URL: ${config.liveBaseUrl || 'https://pay.esicia.rw (default)'}`,
    'cyan'
  );
  ...
  if (config.environment === 'live' && effectiveUrl.includes('esicia.com')) {
    ...
    config.environment === 'live' &&
    effectiveUrl.includes('esicia.rw')
  ) {
    logSuccess('Correctly using LIVE URL for live environment');
  } else if (
    config.environment === 'sandbox' &&
    effectiveUrl.includes('esicia.com')
  ) {
  ...
  if (hasErrors) {
    ...
    log(
      '\n  Your application will NOT work correctly with current settings.',
      'red'
    );
  } else if (hasWarnings) {
