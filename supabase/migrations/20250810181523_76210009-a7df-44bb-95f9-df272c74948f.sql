...
create policy if not exists "Users can upsert own profile" on public.profiles
...
create policy if not exists "Users can view their roles" on public.user_roles
for select to authenticated using (user_id = auth.uid());

create policy if not exists "Admins can view all roles" on public.user_roles
for select to authenticated using (public.has_role(auth.uid(), 'admin'));

-- Do not allow insert/update/delete by clients by default (no policies for those)
